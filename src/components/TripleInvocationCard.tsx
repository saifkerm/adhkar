  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="rounded-lg border p-4 text-center text-xl font-extrabold">
          {inv.transcription}
        </div>
          ...
          {inv.parts.map((p) => {
            ...
            return (
              <div key={p.key} className="rounded-xl border p-3">
                <div className="mb-2 text-center text-sm font-extrabold">{p.label} • {g}</div>
                <div className="mb-2 flex items-center gap-3">
                  <Progress value={partPct} className="h-2" />
                  <span className="tabular-nums text-sm font-bold">{val} / {g}</span>
                </div>
                <div className="flex gap-2">
                  <Button className="h-12 flex-1 text-base" onClick={() => update(p.key, val + 1, g)}>+1</Button>
                  <Button variant="secondary" className="h-12" onClick={() => update(p.key, 0, g)}>↺</Button>
                </div>
              </div>
            )
          })}
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="tabular-nums text-sm font-bold">{current} / {goalTotal}</span>
        </div>
      </CardContent>
    </Card>
  )
