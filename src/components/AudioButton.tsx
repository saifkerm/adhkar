...
export function AudioButton({ id }: { id: string }) {
  ...
  return (
    <Button
      onClick={onClick}
      className={`h-10 rounded-full px-3 font-extrabold text-white ${playing ? "bg-red-700" : "bg-red-600"}`}
      variant="default"
      aria-label={playing ? "Pause" : "Lire"}
    >
      {playing ? <PauseIcon className="mr-2 h-4 w-4" /> : <PlayIcon className="mr-2 h-4 w-4" />}
      {playing ? "Pause" : "Lire"}
    </Button>
  )
}